...
const Index = () => {
  ...
  const { days, hours, minutes, seconds } = useCountdown(eventStart);

  const handleRegister: React.FormEventHandler<HTMLFormElement> = (e) => {
    e.preventDefault();
    toast({ title: "Inscrição enviada!", description: "Em breve entraremos em contato por e-mail." });
  ...
  const jsonLd = {
    ...
    description:
      "Participe do Hackathon Tech Sprint 2025: 24h de inovação, prêmios, networking e muito aprendizado.",
      ...
      <main id="topo">
            ...
            <div>
              ...
              <div className="mt-8 flex flex-wrap gap-3">
                <Button asChild size="lg">
                  <a href="#inscricao" className="inline-flex items-center gap-2">Inscreva-se agora <ArrowRight className="h-4 w-4" /></a>
                </Button>
            ...
            <div id="video">
              <AspectRatio ratio={16 / 9}>
                <iframe
                  ...
                  title="Vídeo do Hackathon Tech Sprint"
                  loading="lazy"
        ...
        <section id="beneficios" className="container py-16 md:py-24">
          ...
          <div className="mt-8 grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
            {[{
              icon: <Trophy className="h-6 w-6" />, title: "Prêmios", desc: "Premiações para os melhores projetos, incluindo bolsas e gadgets."
            },{
              icon: <Users className="h-6 w-6" />, title: "Networking", desc: "Conecte-se com profissionais, empresas e outros devs."
            },{
              icon: <GraduationCap className="h-6 w-6" />, title: "Aprendizado", desc: "Workshops e mentorias com especialistas do mercado."
            },{
              icon: <Clock className="h-6 w-6" />, title: "Experiência", desc: "Desenvolva habilidades práticas trabalhando em equipe sob pressão."
            }].map((b) => (
              <Card key={b.title}>
                <CardHeader>
        ...
        <section id="agenda" className="container py-16 md:py-24">
          ...
          <div className="mt-8 grid gap-6 lg:grid-cols-3">
            {[
              { time: "09:00", title: "Abertura & Boas-vindas", desc: "Credenciamento e apresentação das regras." },
              { time: "10:00", title: "Workshop #1", desc: "Boas práticas de prototipagem rápida." },
              { time: "12:30", title: "Almoço", desc: "Intervalo e networking." },
              { time: "14:00", title: "Início do Hacking", desc: "Formação dos times e kick-off." },
              { time: "20:00", title: "Mentorias", desc: "Sessões com especialistas." },
              { time: "09:00 (Dia 2)", title: "Pitch & Demonstrações", desc: "Apresentação dos projetos finalistas." },
            ].map((a) => (
              <Card key={a.time}>
                <CardHeader>
        ...
        {/* Patrocinadores */}
        <section id="patrocinadores" className="container py-16 md:py-24">
          <h2 className="text-3xl font-semibold tracking-tight">Patrocinadores e Parceiros</h2>
          <p className="mt-2 text-muted-foreground">Apoio de empresas que aceleram a inovação.</p>

          <div className="mt-8 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-6 items-center">
            {[1,2,3,4,5,6].map((i) => (
              <a key={i} href="#" aria-label={`Patrocinador ${i}`} className="opacity-70 hover:opacity-100 transition">
                <img src="/placeholder.svg" alt={`Logo do patrocinador ${i} do Hackathon`} loading="lazy" className="h-10 w-auto mx-auto" />
              </a>
            ))}
          </div>
        </section>
        ...
        {/* Testemunhos */}
        <section id="testemunhos" className="container py-16 md:py-24">
          <h2 className="text-3xl font-semibold tracking-tight">O que dizem os participantes</h2>
          <p className="mt-2 text-muted-foreground">Experiências de edições anteriores.</p>

          <div className="mt-8 grid gap-6 md:grid-cols-2 lg:grid-cols-3">
            {[
              { name: "Ana Lima", text: "Foi transformador! Conheci pessoas incríveis e aprendi muito.", initials: "AL" },
              { name: "João Silva", text: "A dinâmica do evento é excelente. Recomendo demais!", initials: "JS" },
              { name: "Marina Souza", text: "Sair com um protótipo funcionando em 24h foi sensacional.", initials: "MS" },
            ].map((t) => (
              <Card key={t.name}>
